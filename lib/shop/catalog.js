...
// Version éditée par Souhila (Espace admin), stockée dans le store studio.
// Absente ou invalide → on sert le catalog.json du repo comme avant.
function shopOverride() {
  let doc = null;
  try {
    doc = require("../studio/store").loadShopDoc();
  } catch (_err) {
    return null; // store supabase non hydraté : fallback fichier
  }
  if (!doc || !Array.isArray(doc.products) || !doc.products.length) return null;
  try {
    assertCatalog(doc);
  } catch (err) {
    console.error("[shop] catalogue édité invalide, fichier utilisé :", err.message);
    return null;
  }
  return doc;
}
...
function loadCatalog() {
  return shopOverride() || fileCatalog();
}
